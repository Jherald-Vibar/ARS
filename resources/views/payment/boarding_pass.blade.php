...
<div class="mt-6 text-center">
    @if (session('success'))
        <div class="flex justify-center mt-6">
            <p class="flex items-center gap-2 text-lg font-semibold text-green-600">
                <i class="fas fa-check-circle"></i>
                {{ session('success') }}
            </p>
        </div>
    @endif
    <p class="mt-2 text-lg font-bold">Thanks for booking with us — premium travel awaits.</p>
</div>

@foreach ($boardingPasses as $pass)
    <div class="w-full max-w-3xl mx-auto mt-6 bg-white border shadow-lg rounded-xl">
        <div class="flex justify-between px-4 py-2 text-white bg-blue-900 rounded-t-xl">
            <h2 class="font-bold">BOARDING PASS</h2>
        ...
        <div class="flex w-full h-64 max-w-3xl text-zinc-50">
            <div class="flex items-center justify-center h-full px-8 bg-zinc-900 rounded-l-3xl">
            ...
            <div class="flex flex-col flex-grow h-full px-10 py-8 bg-zinc-900 rounded-r-3xl">
                <div class="flex items-center justify-between w-full">
                    <div class="flex flex-col items-center">
                        <span class="text-4xl font-bold">{{$pass->flight->departureAirport->city}}</span>
                        <span class="text-sm text-zinc-500">{{$pass->flight->departureAirport->country}}</span>
                    </div>
                    @php
                        preg_match('/\d+$/', $pass->flight->flight_number, $numberMatch);
                        $number = $numberMatch[0] ?? '';

                        $lettersOnly = preg_replace('/\d+/', '', $pass->flight->flight_number);
                        $consonants = preg_replace('/[aeiou\s]/i', '', $lettersOnly);

                        $flightNumber = strtoupper($consonants) . $number;
                    @endphp
                    @php
                        $departure = \Carbon\Carbon::parse($pass->flight->departure_time);
                        $arrival = \Carbon\Carbon::parse($pass->flight->arrival_time);
                        $diffInMinutes = $arrival->diffInMinutes($departure);
                        $hours = floor($diffInMinutes / 60);
                        $minutes = $diffInMinutes % 60;
                    @endphp
                    <div class="flex flex-col items-center flex-grow px-10">
                        <span class="text-xs font-bold">{{$flightNumber}}</span>
                        ...
                        <div class="flex items-center w-24 h-8 px-3 mt-2 rounded-full bg-lime-400">
                            <span class="text-sm font-bold text-center text-zinc-900">{{ $hours }}h : {{ $minutes }}m</span>
                        </div>
                    </div>
                    <div class="flex flex-col items-center">
                        <span class="text-4xl font-bold">{{$pass->flight->arrivalAirport->city}}</span>
                        <span class="text-sm text-zinc-500">{{$pass->flight->arrivalAirport->country}}</span>
                    </div>
                </div>
                <div class="flex justify-between w-full mt-auto">
                    <div class="flex flex-col">
                        <span class="text-xs text-zinc-400">Date</span>
                        <span class="font-mono">{{ now()->format('Y-m-d') }}</span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-xs text-zinc-400">Departure</span>
                        <span class="font-mono">{{Carbon\Carbon::parse($pass->flight->departure_time)->format('h:i A')}}</span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-xs text-zinc-400">Passenger</span>
                        <span class="font-mono">{{$pass->passenger->full_name}}</span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-xs text-zinc-400">Seat</span>
                        <span class="font-mono">{{$pass->passenger->seat_number}}</span>
                    </div>
                </div>
            </div>
        </div>
...
<div class="mt-8 text-center">
    <a href="{{ route('receipt-pdf', ['bid' => $booking->id])}}"  class="px-6 py-2 text-white bg-gray-800 rounded hover:bg-gray-700">
        Click Here To Print E-Ticket Receipt & Itinerary
    </a>
</div>
